...
The product helps students, parents, and counselors make data-informed college-major and university decisions using trusted admissions, academic, ranking, outcome, and collaboration workflows.
...
- Do not fabricate admissions, ranking, school, program, salary, outcome, or source data.
- Product surfaces must preserve data provenance and show verification cues where users rely on factual claims.
- Data assets are first-class product capabilities, not hidden implementation details.
- User experiences consume data contracts rather than redefining source truth locally.
- Commercial packaging may control access, but it must not weaken data authenticity rules.
