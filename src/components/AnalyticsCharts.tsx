...
export default function AnalyticsCharts({ onSelectField, selectedFieldId, language }: AnalyticsChartsProps) {
  ...
  const displayFields = [...detailedFields].sort((a, b) => {
  ...
  const maxEarnings = Math.max(...detailedFields.map(f => f.primeMedianEarningsVal));
  const maxGrowth = Math.max(...detailedFields.map(f => Math.abs(f.degreeProductionChangePercent)));
  const maxUnemp = Math.max(...detailedFields.map(f => Math.max(f.unemploymentRecentPercent, f.unemploymentPrimePercent)));
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 border-b border-slate-200 pb-5 font-sans">
        <div>
          <h3 className="text-xl md:text-2xl font-bold tracking-tight text-slate-900 flex items-center gap-2">
            ...
            {language === 'zh' ? '交互式核心指标可视化' : 'Interactive Analytics & Metrics Dashboard'}
          </h3>
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl self-start md:self-center border border-slate-200 shadow-inner">
          ...
          >
            ...
            {language === 'zh' ? '热度趋势' : 'Degree Growth'}
          </button>
          ...
          >
            ...
            {language === 'zh' ? '失业率对比' : 'Unemployment'}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="col-span-1 lg:col-span-8 space-y-3">
          <div className="flex justify-between items-center text-xs text-slate-400 px-1 border-b border-slate-200 pb-1.5 font-semibold">
            <span>{language === 'zh' ? '细分学科大类名称' : 'Detailed Academic Field'}</span>
            <span>
              {activeTab === 'earnings' && (language === 'zh' ? '黄金工作期中位数年薪' : 'Prime-Age Median Year Earnings')}
              {activeTab === 'growth' && (language === 'zh' ? '毕业生规模变动 (2009-2023)' : 'Demand Growth Trend (2009–2023)')}
              {activeTab === 'unemployment' && (language === 'zh' ? '失业率率对照 (近期 vs 黄金工作期)' : 'Unemployment Comparison (Recent vs Prime-Age)')}
            </span>
          </div>
                  ...
                  <div className="flex justify-between items-center mb-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-slate-400 group-hover:text-amber-300 transition-colors uppercase tracking-wider">
                        {belongsBroadField ? (language === 'zh' ? belongsBroadField.nameZh.substring(0, 8) : belongsBroadField.nameEn) : ''}
                      </span>
                    ...
                    {activeTab === 'growth' ? (
                      ...
                      <div className="w-full h-full relative flex items-center">
                        ...
                        <div
                          ...
                          title={`${language === 'zh' ? '黄金年龄失业率' : 'Prime-age Unemployment'}: ${field.unemploymentPrimePercent}%`}
                        ></div>
                      </div>
                    ) : (
                      // Single Standard Bar
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 h-full flex flex-col justify-between">
            <div>
              <div className="px-3 py-1 bg-white border border-slate-200 text-[10px] text-blue-600 rounded-full font-mono font-semibold uppercase tracking-wider inline-block mb-3 shadow-xs">
                {language === 'zh' ? '当前维度要点透视' : 'Dynamic Insight Summary'}
              </div>

              {activeTab === 'earnings' && (
                <div className="space-y-4">
                  <h4 className="font-bold text-slate-900 text-base">
                    {language === 'zh' ? '💸 薪资高地特征' : '💸 Earnings Ceiling'}
                  </h4>
              ...
              {activeTab === 'growth' && (
                <div className="space-y-4">
                  <h4 className="font-bold text-slate-900 text-base">
                    {language === 'zh' ? '📈 选科意愿两极化' : '📈 Dynamic Talent Shift'}
                  </h4>
                  <p className="text-slate-600 text-sm leading-relaxed">
                    {language === 'zh'
              ...
              {activeTab === 'unemployment' && (
                <div className="space-y-4">
                  <h4 className="font-bold text-slate-900 text-base">
                    {language === 'zh' ? '💼 缓冲带与起跑线' : '💼 Youth Grads Buffer Layer'}
                  </h4>
                  <p className="text-slate-600 text-sm leading-relaxed">
                    {language === 'zh'
                  ...
                  <div className="flex items-center gap-2 mt-2 py-2 px-3 bg-white border border-slate-200 rounded-xl">
                    ...
                    <span className="text-xs text-slate-600">
                      {language === 'zh' ? '蓝色圆点标记：黄金成熟期失业率' : 'Blue Nodes: Prime-Age stable unemployment'}
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="border-t border-slate-200 pt-4 mt-6">
              <div className="bg-white border border-slate-150 rounded-xl p-3 text-xs flex flex-col gap-1.5 text-slate-600">
                <span className="font-semibold text-slate-800">{language === 'zh' ? '📊 本次统计基准年龄定义：' : '📊 Demographics Baseline:'}</span>
                <span>• {language === 'zh' ? '近期毕业生 (Recent Grads): 22–26岁' : 'Recent College Grads: Ages 22–26'}</span>
                <span>• {language === 'zh' ? '黄金劳动年龄 (Prime-Age): 25–54岁' : 'Prime-Age Workers: Ages 25–54'}</span>
              </div>
            </div>
          </div>
        </div>
